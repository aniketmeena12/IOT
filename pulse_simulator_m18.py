#!/usr/bin/env python3
"""
Printing Machine Pulse Simulator — ARINO M18 Compatible
========================================================
Simulates an ARINO M18 inductive proximity sensor mounted on a printing
...
Each pulse is written to InfluxDB with fields that support:
  - batch counting, cycle speed, dashboard, and alerting

Measurement: spring_pulses (compatible with existing Grafana dashboard)
Tags:
    machine         e.g. "m18_printer_01"
    series          product batch number
...
MACHINE_NAME = os.getenv("MACHINE_NAME", "m18_printer_01")
...
def _shutdown(signum, frame):
    global running
    print(f"\n[M18 Simulator] Signal {signum} — shutting down gracefully...")
    running = False
...
# ---------------------------------------------------------------------------
def main():
    print(f"[M18 Simulator] ARINO M18 Printing Machine Pulse Simulator")
    print(f"[M18 Simulator] Machine : {MACHINE_NAME}")
    print(f"[M18 Simulator] Speed   : {SHEETS_PER_SEC_MIN}–{SHEETS_PER_SEC_MAX} sheets/sec")
    print(f"[M18 Simulator] Job size: {SHEETS_PER_JOB_MIN}–{SHEETS_PER_JOB_MAX} sheets/job")
    print(f"[M18 Simulator] Faults  : missed={MISSED_PULSE_PROB*100:.1f}%  double={DOUBLE_PULSE_PROB*100:.2f}%")
    print(f"[M18 Simulator] InfluxDB: {INFLUXDB_URL}")
    ...
    for attempt in range(15):
        try:
            ...
            if health.status == "pass":
                print("[M18 Simulator] InfluxDB is healthy — starting simulation.")
                break
        except Exception as exc:
            print(f"[M18 Simulator] Waiting for InfluxDB (attempt {attempt+1}/15): {exc}")
            time.sleep(2)
    else:
        print("[M18 Simulator] WARNING: Could not confirm InfluxDB health — proceeding anyway.")
    ...
    while running:
        ...
        # Pick a base speed for this job
        base_speed_sps = random.uniform(SHEETS_PER_SEC_MIN, SHEETS_PER_SEC_MAX)
        base_gap_sec   = 1.0 / base_speed_sps

        print(f"\n[M18 Simulator] ── Job #{job_number} (Series #{series_count}) ──")
        print(f"[M18 Simulator]    Sheets   : {total_sheets}")
        print(f"[M18 Simulator]    Speed    : {base_speed_sps:.1f} sheets/sec  ({base_speed_sps*60:.0f}/min)")
        print(f"[M18 Simulator]    Shift    : {current_shift()}")

        sheet_in_job = 0   # pulses written for this job so far
        ...
        for sheet_idx in range(total_sheets):
            ...
            # ── Fault simulation ─────────────────────────────────────────
            # Double pulse: mechanical vibration
            if random.random() < DOUBLE_PULSE_PROB:
                print(f"[M18 Simulator]    ⚠ Sheet {sheet_idx+1}: DOUBLE pulse")
                # Write first pulse
                write_pulse(write_api, job_number, series_count,
                    sheet_number=sheet_idx+1, job_total_so_far=sheet_in_job,
                    cycle_ms=cycle_ms, sheets_per_min=spm)
                # Write second pulse 2–5 ms later (vibration echo)
                echo_delay = random.uniform(0.002, 0.005)
                time.sleep(echo_delay)
                write_pulse(write_api, job_number, series_count,
                    sheet_number=sheet_idx+1, job_total_so_far=sheet_in_job,
                    cycle_ms=echo_delay*1000, sheets_per_min=60_000/(echo_delay*1000))
                last_pulse_ts = time.perf_counter()
                continue

            # Missed pulse: sensor blind-spot
            if random.random() < MISSED_PULSE_PROB:
                print(f"[M18 Simulator]    ⚠ Sheet {sheet_idx+1}: MISSED pulse")
        ...
        # ── Job complete — operator pause for loading ─────────────────────
        pause_sec = random.uniform(JOB_PAUSE_SEC_MIN, JOB_PAUSE_SEC_MAX)
        print(f"[M18 Simulator]    Job #{job_number} done — {sheet_in_job} sheets printed")
        print(f"[M18 Simulator]    Operator pause: {pause_sec:.1f} sec (loading next stack)")

        # Sleep in 100 ms chunks so Ctrl+C is responsive
    ...
    write_api.close()
    client.close()
    print("[M18 Simulator] Shutdown complete.")
